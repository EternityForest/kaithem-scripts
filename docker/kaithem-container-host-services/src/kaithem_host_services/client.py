...
class UDSClient:
    ...
    def _connect_and_verify(self) -> socket.socket:
        ...
        if pid <= 0:
            s.close()
            raise InsecureConnectionError(f"socket peer pid={pid} looks invalid")
        return s
    ...
    def post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise AuthServerUnavailable(f"bad status line: {status_line!r}") from e
...
def _verify_socket_file(path: Path, expected_uid: int) -> None:
    ...
    if nlinks != 1:
        raise InsecureConnectionError(f"{path} has st_nlink={nlinks}; refusing hardlinked socket")
...
def check_password(
    ...
    *,
    ...
    resp = client.post_json("/check_password", {"user": username, "password": password})
